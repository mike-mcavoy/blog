...
public class StrongTextParser extends BaseParser {

    @Override
    public Optional<Node> match(List<Token> tokens) {
        boolean found = this.findTypes(tokens, new ArrayList<TokenType>(Arrays.asList(
                TokenType.STAR, TokenType.STAR, TokenType.TEXT, TokenType.STAR, TokenType.STAR)));

        if (found) {
            Node textNode = new Node();
            textNode.setType(Type.STRONG);
            textNode.setValue(tokens.get(2).value());
            textNode.setConsumed(5);
